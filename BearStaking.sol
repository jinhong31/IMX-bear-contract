...
contract BearFarm is Ownable {
  ...
  function batchStake(uint[] memory tokenIds) external payable {
    ...
    for (uint256 i = 0; i < tokenIds.length; i++) {
      require(bearNft.ownerOf(tokenIds[i]) == msg.sender, 'you are not owner!');
      ownerOfToken[tokenIds[i]] = msg.sender;
      bearNft.transferFrom(msg.sender, address(this), tokenIds[i]);
  ...
  function batchWithdraw(uint[] memory tokenIds) external payable {    
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      ...
      bearNft.transferFrom(address(this), msg.sender, tokenIds[i]);
